...
- Never run destructive or irreversible commands (prod deploys, migrations against shared environments, `kubectl delete` of live resources) without explicit approval — ask.
- Cluster access requires the university VPN; if `kubectl` is unreachable, note it as UNVERIFIED and move on — do not guess cluster state.
