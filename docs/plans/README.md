...
|---|---|---|---|
| [2026-09-02-db-recovery.md](active/2026-09-02-db-recovery.md) | fix/76-db-recovery | — | **Planned** — CNPG postgres recovery to declared 1 instance (#76): WAL-archive deadlock diagnosed (`Expected empty archive` from stale WALs in MinIO), execution steps prepared (reversible move-aside, operator scale-down, approval-required force-removal, manual backup). Docs-only wave; mutations await lead execution + user approval |
| [2026-09-02-proxy-502.md](active/2026-09-02-proxy-502.md) | fix/75-proxy-502 | — | **Active** — kubelet streaming-proxy 502 diagnosed (RKE2 supervisor proxy `127.0.0.1:9345` has no remotedialer session for worker-02; root cause of 90-day CNPG backup outage). Remediation commands prepared; lead executes with user approval |
