...
|---|---|---|---|
| [2026-09-03-architecture-and-dr.md](active/2026-09-03-architecture-and-dr.md) | — | — | **Active** — architecture deep-dive + failure-mode matrix + DR findings (InvenioRDM vs DBRepo, data plane, 10 findings, restore-drill roadmap). Operator actions pending (sealed-secrets key backup, machineID IT ticket, off-site storage decision, SMTP creds) |
| [2026-09-02-db-recovery.md](active/2026-09-02-db-recovery.md) | fix/76-db-recovery | #77 | **EXECUTED + VERIFIED 2026-09-03** — WALs moved aside (42 objects, reversible), archiver unblocked, operator reconciled to **1 instance (healthy state)**, postgres-1/2 removed, conditions Ready+ContinuousArchiving True. Manual backup still blocked by #75 (502) |
| [2026-09-02-proxy-502.md](active/2026-09-02-proxy-502.md) | fix/75-proxy-502 | #78 | **In progress** — agent restarted (cluster-level + node edit-save in Rancher UI) but worker-02 session still NOT registered. Root cause: machineID collision (all 3 nodes share `de88ca16...`). Durable fix needs VM console (university IT); affinity workaround proposed |
