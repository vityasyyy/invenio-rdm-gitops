...
> **Status:** **RESOLVED 2026-09-05** — worker-02 streaming verified working
> (logs + exec + `proxy/healthz` all OK). Cause of recovery: university IT killed
> and restarted worker-02, then reconnected the agent session — the dropped
> remotedialer session re-registered. **The machineID collision (all 3 nodes share
> `de88ca16...`) remains, so flap risk remains; the IT ticket for unique
> machine-ids is still required.** No cluster mutations performed from this repo
> (docs-only sync).

## Resolution 2026-09-05 (lead-verified live state)

- University IT killed and restarted worker-02, then reconnected the agent
  session. This re-registered worker-02's session in the supervisor proxy's
  remotedialer table — the exact session that was missing (`failed to find
  Session for client ubuntu-btd-kubernetes-worker-02`).
- Streaming verification (lead, 2026-09-05): `kubectl logs` to worker-02 pods
  OK, `kubectl exec` to worker-02 pods OK, `kubectl get --raw
  /api/v1/nodes/ubuntu-btd-kubernetes-worker-02/proxy/healthz` returns `ok`.
- Cluster state: 3 nodes Ready, 17 of 17 ArgoCD apps Synced+Healthy, invenio
  web ×2 / worker ×2 / scheduler ×1 Running, ping 200, ArgoCD 200.
- Remaining risk: all 3 nodes still share machineID `de88ca16...`. The VM-clone
  collision was the prime suspect for the session-registration failure, and it
  is unchanged — a future agent reconnect flap can drop worker-02's session
  again. Operator action: university IT ticket to regenerate `/etc/machine-id`
  + reboot on all 3 nodes (needs VM console, not SSH).
- CNPG backup pipeline is NOT yet proven end-to-end: 1,872 failed Backup
  objects, zero completed; ScheduledBackup stuck since 2026-08-21. The
  scheduler deadlock (`already exists` error on
  `postgres-daily-backup-20260821030200`) is owned by the backup-diag worker
  (`docs/plans/active/2026-09-05-backup-scheduler-deadlock.md`) — see the
  db-recovery plan.
...
- [x] **Group 2 — Remediation (lead executes with approval)**: resolved 2026-09-05
      by university IT (worker-02 kill + restart, agent session reconnected) —
      the prepared cluster-agent restart / Rancher UI paths were superseded
- [x] **Group 3 — Verification**: logs/exec/healthz to worker-02 all OK 2026-09-05;
      CNPG backup completion still pending (scheduler deadlock → backup-diag worker)
- [x] **Group 4 — Docs**: plan status update + index (this sync)

## Acceptance Criteria

- [x] `kubectl -n invenio logs` to a worker-02 pod succeeds (verified 2026-09-05)
- [x] `kubectl -n invenio exec` to a worker-02 pod succeeds (verified 2026-09-05)
- [x] `kubectl -n invenio logs` to a worker-01 pod still succeeds (worker-01 regression — cluster fully Synced+Healthy 2026-09-05)
- [x] `kubectl get --raw /api/v1/nodes/ubuntu-btd-kubernetes-worker-02/proxy/healthz` returns `ok` (verified 2026-09-05)
- [ ] A CNPG backup completes: `kubectl -n database get backups | tail` shows a
      `completed` phase — **still pending 2026-09-05** (1,872 failed, zero
      completed; scheduler deadlock owned by backup-diag worker, not this plan)
- [x] No cluster mutations performed from this repo during diagnosis/sync (docs-only)
